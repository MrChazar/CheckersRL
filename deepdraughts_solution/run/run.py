...
def play_with_DQN(play_using_white=True, checkpoint=None, use_gpu=True):
    gui = GUI()
    ...
    if checkpoint and os.path.exists(checkpoint):
        ...
        try:
            model_container = Model.load(checkpoint)
        ...
        # We create new model
        model_container = Model(env_args, use_gpu=use_gpu)
        policy_net = model_container.policy_net
    ...
    # creation of player
    dqn_player = DQNAgent(model_net=policy_net, epsilon=0.0, use_gpu=use_gpu)
